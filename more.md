...
Некоторые полезные специальные методы перечислены ниже. Все другие методы можно посмотреть в документации.
...
[]
>>>assert len(mylist) >= 1 
Traceback(mostrecentcalllast): File"<stdin>", line1, in <module> AssertionError
```
